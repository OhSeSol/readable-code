...
public class StudyCafeFileHandler {
    ...
    public StudyCafeLockerPass getSelectedTypeLockerPass(StudyCafePass studyCafePass) {
        return readLockerPasses().stream()
                .filter(option ->
                        option.getPassType() == studyCafePass.getPassType()
                                && option.getDuration() == studyCafePass.getDuration()
                )
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("잘못된 이용권 정보입니다."));
    }
}
